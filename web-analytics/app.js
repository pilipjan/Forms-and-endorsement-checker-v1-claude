(function () {
  ...
  // Analytics shorthand — analytics.js may not be loaded (e.g. running the
  // plain offline build), so this is always safe to call.
  function T(event, data) {
    if (window.ComparatorAnalytics) window.ComparatorAnalytics.track(event, data);
  }
  T("session_start", { ua: navigator.userAgent, lang: navigator.language });
  ...
  function compare() {
    ...
    T("compare_run", {
      previousCount: state.previousItems.length,
      currentCount: state.currentItems.length,
      docsCompared: cmps.length,
      statusCounts: summary,
      durationMs: Math.round(performance.now() - t0),
      // The actual raw text of anything the parser couldn't handle — this is
      // the highest-value signal for improving parser.js regex coverage.
      unknownLines: state.results.filter(r => r.status === "Unknown Format").map(r => r.originalCurrent || r.originalPrevious || r.normalizedCode),
      duplicateLines: state.results.filter(r => r.duplicateCount).flatMap(r => r.duplicateLines),
    });
  }
  ...
  function downloadFile(name, content, mime) {
    ...
    T("export", { format: ext, mime, rowCount: state.results.length, name });
  }
  ...
  function handleLinkClick(rowId) {
    ...
    merged._id = rowA._id;
    T("manual_link", { statusA: rowA.status, statusB: rowB.status, codeA: rowA.normalizedCode, codeB: rowB.normalizedCode });
  ...
  function saveNote() {
    ...
    if (v) { state.rowNotes.set(noteTarget, v); T("note_added", { code: noteTarget, length: v.length }); }
  ...
  el.resultsBody.addEventListener("blur", e => {
    ...
    scheduleDraftSave();
    if (field === "displayCode" || field === "description") T("manual_edit", { code, field, status: orig && orig.status });
  }, true);
